...
class GerenciadorTelas {
    ...
    private void desenharBannerTitulo(String texto, float bannerY, Sketch sketch) {
        ...
        if (fonteMenu != null) sketch.textFont(fonteMenu);
    ...
    private void desenharMenuInicial(Sketch sketch) {
        ...
        if (fonteNativa != null) sketch.textFont(fonteNativa);
        else sketch.textFont(sketch.createFont("SansSerif", 13));
    ...
    private void desenharCardMapa(String rotulo, String arquivo, PImage thumbnail, float x, float y, int indiceOpcao, Sketch sketch) {
    ...
    private void executarAcaoOpcao(int opcao, Sketch sketch) {
        switch (estadoAtual) {
            case MENU_INICIAL:
                if (opcao == 0) mudarEstado(EstadoTela.SELECAO_MAPA, 4);
                else if (opcao == 1) mudarEstado(EstadoTela.CREDITOS, 1);
                else if (opcao == 2) sketch.exit();
                break;

            case PAUSADO:
                if (opcao == 0) mudarEstado(EstadoTela.EM_EXECUCAO, 3);
                else if (opcao == 1) mudarEstado(EstadoTela.EM_EXECUCAO, 3); // Lógica de Reset
                else if (opcao == 2) mudarEstado(EstadoTela.MENU_INICIAL, 3);
                break;

            case SELECAO_MAPA:
                if (opcao == 0) mapaSelecionado = "mapa1.txt";
                else if (opcao == 1) mapaSelecionado = "mapa2.txt";
                else if (opcao == 2) mudarEstado(EstadoTela.EM_EXECUCAO, 3);
                else if (opcao == 3) mudarEstado(EstadoTela.MENU_INICIAL, 3);
                break;

            case CREDITOS:
                if (opcao == 0) mudarEstado(EstadoTela.MENU_INICIAL, 3);
                break;
        }
    }
